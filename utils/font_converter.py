...
class FontConverter:
    ...
    def get_char(self, char: int) -> np.ndarray:
        ...
        return self.image[
            y_offset : y_offset + self.char_height,
            x_offset : x_offset + self.char_width,
        ]
    ...
    def _count_touches(
        self,
        char1_data: np.ndarray,
        char2_data: np.ndarray,
        char2_start_pos: int,
        overlap_top: int,
        overlap_bottom: int,
    ) -> tuple[int, int]:
        ...
        ownership = np.zeros(
            (char1_height, total_width), dtype=np.uint8
        )  # 1=char1, 2=char2
            ...
            for col in range(total_width):
                if combined[row, col] == 1:
                    ...
                    for dr, dc in [(1, 1), (1, -1), (-1, 1), (-1, -1)]:
                        ...
                        if (
                            0 <= new_row < char1_height
                            and 0 <= new_col < total_width
                            and combined[new_row, new_col] == 1
                        ):
                            neighbor_owner = ownership[new_row, new_col]

                            # Only count touches between different characters
                            if (
                                pixel_owner != neighbor_owner
                                and pixel_owner > 0
                                and neighbor_owner > 0
                            ):
                                # Create a unique key for this touch pair to avoid double counting
                                touch_key = tuple(
                                    sorted([(row, col), (new_row, new_col)])
                                )
    ...
    def _check_collision(
        ...
        for row in range(char1_height):
            for char2_col in range(char2_width):
                ...
                if (
                    0 <= char2_pixel_pos < char1_width
                    and char1_data[row, char2_pixel_pos] == 1
                    and char2_data[row, char2_col] == 1
                ):
                    return True
    ...
    def compute_kerning(
        self, char1_index: int, char2_index: int, default_kerning: int = 1
    ) -> int:
    ...
    def render_kerned_string(
        self,
        text_bytes: bytes,
        default_kerning: int = 1,
        external_kerning_table: dict = None,
        external_width_table: bytes = None,
    ) -> np.ndarray:
        """
        ...
        for i in range(len(text_bytes) - 1):
            ...
            if (
                ...
                and (char1_index, char2_index) in external_kerning_table
            ):
                ...
                kerning = (
                    self.compute_kerning(char1_index, char2_index, default_kerning) + 1
                )
    ...
    def render_string(self, text_bytes: bytes) -> np.ndarray:
        ...
        for i, char in enumerate(chars):
            ...
            char_top_row = (
                np.argmax(char_rows_with_pixels) if np.any(char_rows_with_pixels) else 0
            )
            char_bottom_row = (
                len(char_rows_with_pixels) - 1 - np.argmax(char_rows_with_pixels[::-1])
                if np.any(char_rows_with_pixels)
                else self.char_height - 1
            )

            # Place character at its correct vertical position
            result[
                char_top_row : char_bottom_row + 1,
                current_pos : current_pos + char_widths[i],
            ] = char[char_top_row : char_bottom_row + 1, :]
    ...
    def debug_kerned_string(self, text: str, table) -> None:
        ...
        for i in range(len(text_bytes) - 1):
            ...
            print(
                f"Pair {i + 1}: '{char1_str}'{char2_str}' ({char1_index:02x},{char2_index:02x})"
            )
    ...
    def debug_diagonal_touches(self, pair: str, table, test_kerning: int) -> None:
                        ...
                        if (
                            ...
                            if (
                                ...
                                if touch_key not in touch_pairs:
                                    ...
                                    char_type1 = (
                                        "char1" if pixel_owner == 1 else "char2"
                                    )
                                    char_type2 = (
                                        "char2" if neighbor_owner == 2 else "char1"
                                    )
                                    print(
                                        f"Touch: {char_type1}({row},{col}) {direction_name} {char_type2}({new_row},{new_col})"
                                    )
        ...
        print()
        print(
            f"Character boundary: char1 ends at col {char1_width - 1}, char2 starts at col {char2_start_pos}"
        )

    def find_kerning_pairs(
        self, table, test_pairs: List[str]
        ...
        for pair in test_pairs:
            ...
            try:
                optimal_kerning = self.compute_kerning(
                    chars[0], chars[1], default_kerning=1
                )
                # Skip useless entries:
                # - Same as default (no benefit)
                # - Invalid kerning values
                if (
                    optimal_kerning != 1 and optimal_kerning < 0
                ):  # Different from default and valid
                    kerning_pairs[(chars[0], chars[1])] = optimal_kerning
                    char1_str = table.to_text(bytes([chars[0]]))
                    char2_str = table.to_text(bytes([chars[1]]))
                    print(
                        f"  {pair} '{chars[0]}{chars[1]}' ({chars[0]:02x},{chars[1]:02x}): kerning = {optimal_kerning}"
                    )
